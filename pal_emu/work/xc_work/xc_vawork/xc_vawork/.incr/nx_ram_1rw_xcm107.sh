...
module nx_ram_1rw_xcm107 ( clk, rst_n, ovstb, lvm, mlvm, mrdten, bimc_rst_n, bimc_isync, bimc_idat, bimc_odat, bimc_osync, ro_uncorrectable_ecc_error, bwe, din, add, cs, we, dout );
...
input wire logic [37 : 0] bwe ;
input wire logic [37 : 0] din ;
input wire logic [13 : 0] add ;
input wire logic cs ;
input wire logic we ;
output wire logic [37 : 0] dout ;
endmodule
`end_keywords
