...
module cr_kme_fifo_xcm54 ( fifo_in_stall, fifo_out, fifo_out_valid, fifo_overflow, fifo_underflow, clk, rst_n, fifo_in, fifo_in_valid, fifo_out_ack, fifo_in_stall_override );
parameter DATA_SIZE = 256 ;
parameter FIFO_DEPTH = 4 ;
parameter OVERRIDE_EN = 0 ;
parameter STALL_AT = 0 ;
output wire logic fifo_in_stall ;
output wire logic [255 : 0] fifo_out ;
output wire logic fifo_out_valid ;
output wire logic fifo_overflow ;
output wire logic fifo_underflow ;
input wire logic clk ;
input wire logic rst_n ;
input wire logic [255 : 0] fifo_in ;
