...
module nx_ram_1rw_indirect_access_xcm124 ( clk, rst_n, reg_addr, cmnd_op, cmnd_addr, stat_code, stat_datawords, stat_addr, capability_lst, capability_type, wr_stb, wr_dat, rd_dat, ovstb, lvm, mlvm, mrdten, bimc_rst_n, bimc_isync, bimc_idat, bimc_odat, bimc_osync, ro_uncorrectable_ecc_error, hw_add, hw_we, hw_bwe, hw_cs, hw_din, hw_dout, hw_yield );
...
input wire logic [14 : 0] hw_add ;
input wire logic hw_we ;
input wire logic [63 : 0] hw_bwe ;
input wire logic hw_cs ;
input wire logic [63 : 0] hw_din ;
output logic [63 : 0] hw_dout ;
